...
class BookingController {
  async createBooking(req, res, next) {
    try {
      ...
      // Convert coordinates format nếu cần
      
      if (bookingData.pickupLocation?.coordinates?.lat !== undefined) {
        const { lat, lng } = bookingData.pickupLocation.coordinates;
        bookingData.pickupLocation.coordinates = [lng, lat];
      }
      
      if (bookingData.destination?.coordinates?.lat !== undefined) {
        const { lat, lng } = bookingData.destination.coordinates;
        bookingData.destination.coordinates = [lng, lat];
      }
      // Validate request
