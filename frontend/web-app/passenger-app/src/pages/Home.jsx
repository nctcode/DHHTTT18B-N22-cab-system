...
export default function Home() {
    ...
    // Saved places (arrays of addresses)
    const [savedPlaces, setSavedPlaces] = useState(() => {
        try {
            const saved = localStorage.getItem('savedPlaces');
            if (saved) {
                const parsed = JSON.parse(saved);
                // Migrate from old format (single object) to new format (arrays)
                return {
                    home: Array.isArray(parsed.home) ? parsed.home : (parsed.home ? [parsed.home] : []),
                    work: Array.isArray(parsed.work) ? parsed.work : (parsed.work ? [parsed.work] : []),
                };
            }
            return { home: [], work: [] };
        } catch {
    ...
    const [addressType, setAddressType] = useState(null); // 'home' or 'work'
    const [modalView, setModalView] = useState('list'); // 'list' or 'search'
    ...
    const handleAddressSearch = useCallback((value) => {
        ...
        addressSearchTimeout.current = setTimeout(async () => {
            const options = {};
            if (pickupCoords) {
                options.lat = pickupCoords[0];
                options.lon = pickupCoords[1];
            }
    ...
    const handleAddNewAddress = (place) => {
        const newPlace = {
            name: place.name,
            address: place.address,
            location: [place.lat, place.lon],
        };
        const list = [...(savedPlaces[addressType] || []), newPlace];
        const updated = { ...savedPlaces, [addressType]: list };
        setSavedPlaces(updated);
        localStorage.setItem('savedPlaces', JSON.stringify(updated));
        setModalView('list');
    };

    const handleUseSavedAddress = (place) => {
    ...
    const handleDeleteSavedAddress = (type, index) => {
        const list = [...(savedPlaces[type] || [])];
        list.splice(index, 1);
        const updated = { ...savedPlaces, [type]: list };
        setSavedPlaces(updated);
        localStorage.setItem('savedPlaces', JSON.stringify(updated));
    };
        ...
        <div className="relative h-full w-full overflow-hidden bg-gray-100 flex flex-col box-border">
                ...
                <>
                    ...
                    <div className="fixed top-0 left-0 bottom-0 w-72 bg-white z-[800] shadow-2xl animate-slideIn">
                        <div className="bg-gradient-to-br from-primary to-blue-700 px-6 pt-12 pb-6 text-white">
                            ...
                            <p className="font-semibold text-lg">{user?.name || user?.email || 'Người dùng'}</p>
                            <p className="text-blue-200 text-sm">{user?.name ? user.email : (user?.role === 'SUPERADMIN' ? 'Siêu quản trị' : user?.role === 'DRIVER' ? 'Tài xế' : 'Hành khách')}</p>
                        </div>
                        <nav className="py-4">
                            {[
            ...
            <BottomSheet collapsed collapsedHeight={260}>
                ...
                <div className="flex gap-3 mb-5">
                    <button onClick={() => handleSavedPlace('home')}
                        className="flex-1 flex items-center gap-2.5 px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors border border-gray-100 overflow-hidden">
                        ...
                        <div className="text-left min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-400 truncate">
                                {savedPlaces.home.length > 0 ? `${savedPlaces.home.length} địa chỉ đã lưu` : 'Thiết lập địa chỉ'}
                            </p>
                        </div>
                    ...
                    <button onClick={() => handleSavedPlace('work')}
                        className="flex-1 flex items-center gap-2.5 px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors border border-gray-100 overflow-hidden">
                        ...
                        <div className="text-left min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-400 truncate">
                                {savedPlaces.work.length > 0 ? `${savedPlaces.work.length} địa chỉ đã lưu` : 'Thiết lập địa chỉ'}
                            </p>
                        </div>
                        <svg className="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
            ...
            {showAddressModal && (
                <>
                    <div className="fixed inset-0 bg-black/50 z-[900]" onClick={() => setShowAddressModal(false)} />
                    <div className="fixed inset-x-0 bottom-0 z-[950] bg-white rounded-t-3xl shadow-2xl max-h-[80vh] flex flex-col animate-slideUp">
                        {/* Modal Header */}
                        <div className="px-5 pt-5 pb-3">
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                    {modalView === 'search' && (
                                        <button onClick={() => setModalView('list')}
                                    ...
                                    <h3 className="text-lg font-bold text-gray-900">
                                        {addressType === 'home' ? '🏠 Địa chỉ Nhà' : '💼 Nơi làm việc'}
                                    </h3>
                                </div>
                                <button
                            ...
                            {/* Search input (only in search view) */}
                            {modalView === 'search' && (
                                <input
                                    ref={addressInputRef}
                                    type="text"
                                    ...
                                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-blue-100 transition-all placeholder:text-gray-400"
                                    autoComplete="off"
                                />
                            )}
                        </div>
                            ...
                            {modalView === 'list' && (
                                <>
                                    ...
                                    <button
                                        ...
                                        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-50 active:bg-blue-100 rounded-xl transition-colors text-left mb-1"
                                    >
                                        ...
                                        <p className="text-sm font-semibold text-primary">Thêm địa chỉ mới</p>
                                    </button>

                                    {/* Saved addresses */}
                                    {(savedPlaces[addressType] || []).length > 0 && (
                                        <div className="px-4 pt-2 pb-1">
                                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Đã lưu</p>
                                        </div>
                                    )}
                                    {(savedPlaces[addressType] || []).map((place, index) => (
                                        <div key={index}
                                            className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 rounded-xl transition-colors">
                                            <button
                                                onClick={() => handleUseSavedAddress(place)}
                                                className="flex-1 flex items-start gap-3 text-left min-w-0"
                                            >
                                                <div className={`w-10 h-10 ${addressType === 'home' ? 'bg-blue-50' : 'bg-emerald-50'} rounded-full flex items-center justify-center flex-shrink-0 mt-0.5`}>
                                                    <svg className={`w-4 h-4 ${addressType === 'home' ? 'text-primary' : 'text-emerald-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                            ...
                                            <button
                                                onClick={() => handleDeleteSavedAddress(addressType, index)}
                                                className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center hover:bg-red-100 transition-colors flex-shrink-0"
                                            >
                                    ...
                                    {(savedPlaces[addressType] || []).length === 0 && (
                                        <div className="text-center py-8">
                                            <p className="text-3xl mb-2">{addressType === 'home' ? '🏠' : '💼'}</p>
                                            <p className="text-gray-400 text-sm">Chưa có địa chỉ nào được lưu</p>
                                            <p className="text-gray-300 text-xs mt-1">Nhấn "Thêm địa chỉ mới" để bắt đầu</p>
                                        </div>
                                    )}
                                </>
                            )}
                            ...
                            {modalView === 'search' && (
                                <>
                                    ...
                                    {addressResults.map((place) => (
                                        <button
                                            key={place.id}
                                            onClick={() => handleAddNewAddress(place)}
                                            className="w-full flex items-start gap-3 px-4 py-3 hover:bg-gray-50 active:bg-gray-100 rounded-xl transition-colors text-left"
                                        >
                                    ...
                                    {addressQuery && !addressSearching && addressResults.length === 0 && (
                                        <div className="text-center py-10">
                                            <p className="text-3xl mb-2">🔍</p>
                                            <p className="text-gray-400 text-sm">Không tìm thấy địa điểm</p>
                                        </div>
                                    )}
                                    ...
                                    {!addressQuery && (
                                        <div className="text-center py-10">
                                            <p className="text-3xl mb-2">📍</p>
                                            <p className="text-gray-400 text-sm">Nhập địa chỉ để tìm kiếm</p>
                                        </div>
                                    )}
                                </>
                            )}
